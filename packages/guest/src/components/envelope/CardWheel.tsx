 ...
 *
 * Совпадает с тактом роста кучки (--dur-grow в pile.css): карточки встают в
 * барабан ровно тем же движением, которым кучка дорастает до полного размера, —
 * иначе на экране шли бы две разные по длительности анимации одного события.
 */
const DEAL_MS = 900;
...
/**
 * Кривая раздачи — ease-out (кубическая): полная скорость в начале, ноль в конце.
 *
 * Раздача не одна на экране: перед ней кучка дочищает блюр (--dur-fog), и к её
 * началу перед глазами уже стоит чёткая карта в полный размер. Стык этих двух
 * движений и есть всё дело. Кривая с нулём в начале (smootherstep) оставляла
 * между ними мёртвый такт: карта прояснилась и на глаз замерла, а спустя долю
 * секунды разом поехала вниз — те самые «два кадра». Ease-out убирает паузу:
 * барабан трогается ровно в тот кадр, в котором кучка отдала карту, и раскрытие
 * читается продолжением проявления, а не отдельным рывком следом. Конец при этом
 * мягкий — наклон ноль, барабан встаёт, а не втыкается.
 */
...
export function CardWheel({ places, selected, readOnly, state, onSelect }: CardWheelProps) {
  ...
  const paint = useCallback(() => {
    ...
    itemsRef.current.forEach((el, i) => {
      ...
      el.style.opacity = Math.min(1, (CULL - abs) / 12).toFixed(3);
