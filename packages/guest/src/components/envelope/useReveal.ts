 ...
 *
 * Совпадать один в один они при этом не обязаны: число здесь — момент, в
 * который трогается следующий такт, а не длительность перехода, который в этот
 * момент кончается. У кучки они всё же равны, и намеренно (PILE_TIMING.lead
 * против --dur-fog в pile.css): рост там обязан начаться ровно там, где блюр
 * дошёл до нуля. Разводить их можно — переход не обрывается сменой состояния,
 * пока новое правило не меняет его конечное значение, — но кучке это выходило
 * боком, разбор в шапке pile.css.
 */
